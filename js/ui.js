...
function renderSandbox(evaluated) {
  ...
  const shapes = week && week.incomplete ? [] : availableFormations(players);
  const laidOut = week ? week.xi.map((id) => CTX.byId.get(id)).filter(Boolean) : [];
  const shape = week && !week.incomplete && week.xi.length === 11
    ? formationName(xiCounts(laidOut)) : null;
  // A squad still being built has no formation to offer, so the menu shows the
  // shape it currently has rather than an em dash that reads as broken.
  $('#sbFormation').innerHTML = shapes.length
    ? shapes.map((s) => `<option value="${s}"${s === shape ? ' selected' : ''}>${s}</option>`).join('')
    : `<option value="">${laidOut.length ? esc(formationName(xiCounts(laidOut))) : '—'}</option>`;
  $('#sbFormation').disabled = !shape || shapes.length < 2;
  $('#sbFormation').title = week && week.incomplete
    ? `${week.missing} Formations come back once you have a legal eleven.`
    : 'Change formation, keeping as many starters as the new shape allows';
  ...
  if (prefs.sbView === 'list') {
    ...
    // Every player you own appears on this pitch exactly once. Anyone the
    // arrangement did not place goes into his position's line regardless —
    // a player who is off the pitch cannot be sold or replaced, which is how a
    // half-built squad became unrecoverable.
    const placed = new Set([...xi, ...bench].map((p) => p.id));
    const orphans = squadIds.map((id) => CTX.byId.get(id))
      .filter((p) => p && !placed.has(p.id));

    // Each line is ordered by where its occupants sit in the squad, so selling
    // someone leaves a gap exactly where he was instead of shuffling everyone
    // along. A pitch that reflows under the cursor on every sale is what makes
    // clearing four players feel like a glitch.
    const slotOf = new Map((week ? week.squad : []).map((id, i) => [id, i]));
    const lines = { GKP: [], DEF: [], MID: [], FWD: [] };
    const put = (pos, order, html) => { if (lines[pos]) lines[pos].push({ order, html }); };

    [...xi, ...orphans].forEach((p) =>
      put(p.pos, slotOf.has(p.id) ? slotOf.get(p.id) : 99, manCard(p, null, cardOpts(p))));
    // An empty shirt sits in the line it belongs to, so the gap is where the
    // gap is rather than in a footnote.
    holes.forEach((h) => put(h.pos, h.slot == null ? 99 : h.slot, emptyCard(h)));
    Object.keys(lines).forEach((k) => lines[k].sort((a, b) => a.order - b.order));

    const label = shape || `${filled}/15`;
    let html = `<div class="pitch"><span class="shape"${week && week.incomplete
      ? ` title="${esc(week.missing)}"` : ''}>${esc(label)}</span>`;
    ['GKP', 'DEF', 'MID', 'FWD'].forEach((pos) => {
      if (lines[pos].length) {
        html += `<div class="line">${lines[pos].map((e) => e.html).join('')}</div>`;
      }
    });
    html += '</div><div class="bench"><span class="lab">Bench</span><div class="line">' +
      (bench.length ? bench.map((p) => manCard(p, null, cardOpts(p))).join('')
        : `<span class="note">${holes.length
          ? 'No substitutes yet — the eleven comes first.'
          : 'nobody on the bench yet'}</span>`) + '</div></div>';
    $('#sbPitch').innerHTML = html;
  }

  /* ── what to do next, in one line ── */
  const moves = step ? step.transfers.filter((t) => t.in != null).length : 0;
  // A squad that cannot field an eleven says so, above everything else. It is
  // the reason the shape menu is greyed out and the reason the bench is empty,
  // and leaving it unexplained is what made this look like a glitch.
  const short = week && week.incomplete
    ? `<span class="sb-warn">Not a legal eleven yet — ${esc(week.missing)}</span> ` +
      `Everyone you still own is on the pitch; fill the ${holes.length} empty ` +
      `shirt${holes.length === 1 ? '' : 's'} and the formation comes back.`
    : '';

  let hint;
  if (SB_MSG) hint = `<span class="sb-warn">${esc(SB_MSG)}</span>`;
  else if (SB_SEL && SB_SEL.kind === 'slot') {
    hint = `An empty ${esc(SB_SEL.pos)} shirt is selected. Pick anyone from the market to fill it.`;
  } else if (SB_SEL) {
    hint = `${esc((CTX.byId.get(SB_SEL.id) || {}).name || '')} is selected. ` +
      'Pick a replacement, or tap a team-mate to swap them round.';
  } else {
    hint = 'Tap a shirt to select it, <b>×</b> to sell, or tap two players to swap them ' +
      'between the eleven and the bench.' +
      (moves ? ` <b>${moves} transfer${moves === 1 ? '' : 's'} staged.</b>` : '');
  }
  // The shortfall rides alongside whatever else is being said rather than
  // replacing it — it explains the greyed-out shape menu and the empty bench,
  // and those questions do not go away because a slot happens to be selected.
  $('#sbHint').innerHTML = short && !SB_MSG ? `${short} ${hint}` : hint;
  SB_MSG = '';
...
function wireSandbox(week, holes, players, idx) {
  ...
  const pick = (id) => {
    if (SB_SEL && SB_SEL.kind === 'player' && SB_SEL.id !== id) {
      ...
      const r = swapLineup(arr, SB_SEL.id, id, { partial: !!week.incomplete });
  ...
  $$('#sbPitch .man[draggable="true"]').forEach((el) => {
    ...
    el.addEventListener('drop', (e) => {
      ...
      const arr = { xi: week.xi.map((x) => CTX.byId.get(x)).filter(Boolean),
        bench: week.bench.map((x) => CTX.byId.get(x)).filter(Boolean) };
      const r = swapLineup(arr, from, to, { partial: !!week.incomplete });
      if (!r.ok) { SB_MSG = r.error; redraw(); return; }
