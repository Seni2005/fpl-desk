 ...
 *
 * `opts.partial` checks only the maxima. A squad still being built has no
 * eleven yet — complaining that it is short of defenders while you are halfway
 * through buying them is noise, and it would block every legitimate rearrange
 * on the way. The maxima still hold, because two keepers on the pitch is wrong
 * at any stage.
 */
export function shapeProblem(c, opts = {}) {
  const total = c.GKP + c.DEF + c.MID + c.FWD;
  if (c.GKP > 1) return 'Only one goalkeeper can start.';
  ...
  if (c.GKP < 1) return 'You need a goalkeeper in the eleven.';
...
/**
 * A squad that cannot yet field a legal eleven, laid out anyway.
 *
 * Halfway through a wildcard build there is no legal eleven — sell three
 * midfielders and a forward and you are left with nine outfield players, which
 * no formation can accommodate. `optimalXI` correctly returns null for that,
 * and the pitch used to render nothing at all: every player you still owned
 * disappeared, along with any way to sell or replace them.
 *
 * So an incomplete squad gets a provisional layout instead — the best eleven
 * the position MAXIMA allow, ignoring the minima it cannot meet yet, with the
 * rest on the bench. It is explicitly `incomplete`, and `missing` carries the
 * sentence saying what it is still short of, so nothing downstream mistakes it
 * for a side you could actually field.
 */
function provisionalXI(players, value) {
  const MAX = { GKP: 1, DEF: 5, MID: 5, FWD: 3 };
  const byPos = { GKP: [], DEF: [], MID: [], FWD: [] };
  players.forEach((p) => { if (byPos[p.pos]) byPos[p.pos].push(p); });
  Object.keys(byPos).forEach((k) => byPos[k].sort((a, b) => value(b) - value(a)));

  let xi = ['GKP', 'DEF', 'MID', 'FWD'].flatMap((k) => byPos[k].slice(0, MAX[k]));
  // Losing both keepers leaves room for thirteen outfielders under the maxima.
  // Eleven is still the most that can ever start.
  if (xi.length > 11) xi = xi.slice().sort((a, b) => value(b) - value(a)).slice(0, 11);

  const inXI = new Set(xi.map((p) => p.id));
  const ranked = xi.slice().sort((a, b) => value(b) - value(a));
  const counts = xiCounts(xi);
  return {
    formation: null,
    shape: counts,
    xi,
    bench: orderBench(players.filter((p) => !inXI.has(p.id))),
    points: round(xi.reduce((s, p) => s + value(p), 0), 2),
    captain: ranked[0] || null,
    vice: ranked[1] || null,
    manual: false,
    repaired: false,
    incomplete: true,
    missing: shapeProblem(counts),
  };
}
...
export function arrangeXI(squadPlayers, gwIndex = 0, manual = null, key = null) {
  ...
  // A squad with players in it always lays out. Returning null here is what made
  // a half-built wildcard render an empty pitch.
  const fallback = () => provisionalXI(squadPlayers, value);
  const ids = manual && Array.isArray(manual.xi) ? manual.xi : null;
  if (!ids || !ids.length) {
    return auto ? { ...auto, manual: false, repaired: false, incomplete: false } : fallback();
  }
  ...
  if (!built) return auto ? { ...auto, manual: false, repaired: true, incomplete: false } : fallback();
 ...
 * what to do instead.
 */
export function swapLineup(arrangement, aId, bId, opts = {}) {
  // While the squad is short, only the maxima can be enforced — there is no
  // eleven yet to be short of. Refusing every rearrange until the last shirt is
  // filled would make the pitch read-only for the whole of a wildcard build.
  const partial = opts.partial != null ? opts.partial : !!arrangement.incomplete;
  ...
  const problem = shapeProblem(counts, { partial });
  ...
  ctx.gws.forEach((gw, idx) => {
    ...
    moves.forEach((t) => {
      ...
      if (t.in == null) {
        ...
        // fill it with.
        //
        // `slot` is where he sat in the squad, and the empty shirt keeps it. A
        // hole appended to the end of its line instead makes the whole pitch
        // reflow on every sale — which moves the next shirt out from under the
        // cursor, and can put the undo control where the sell control was.
        const slot = squad.indexOf(t.out);
        bank = round(bank + outP.price, 1);
        squad = squad.map((id) => (id === t.out ? null : id));
        holes.push({ out: t.out, pos: outP.pos, name: outP.name, price: outP.price, slot });
        return;
      }
    ...
    weeks.push({
      ...
      // The squad is short of a legal eleven — the layout below it is
      // provisional, and `missing` says what it is short of.
      incomplete: !!(best && best.incomplete),
      missing: best && best.incomplete ? best.missing : null,
